...
import { CreditNote, InvList } from "../../creditNoteLists/index";
...
import { updateappqc } from "../services";
...
interface FormEditCreditNoteProps {
  ...
  itemReportLists: any[]; // Assuming this is an array of item reports
  ReportListId?: number | null; // Optional ReportListId for appqc update
  setAppqc: (appqc: string) => void;
...
FormEditCreditNoteProps) => {
  ...
  const fetchInvoice = async (invoiceId: number) => {
    ...
    if (response) {
      ...
      const itemsForCheck = selectedOrder
        ...
        : null;
      if (itemsForCheck) {
        setIsItemForCheckCredit( itemsForCheck.data?.filter(
            (item: isItemForCheckCredit) =>
              item.creditNoteId != selectedCreditNote?.creditNoteId
          ) || []);
        setIsCNDiscount(
          itemsForCheck.data?.some(
            (item: isItemForCheckCredit) => item.isReturn == 0
          )
        );
        setIsReturn(
          itemsForCheck.data?.some(
            (item: isItemForCheckCredit) => item.isReturn == 1
          )
        );
      }
      setLoading(false);
  ...
  const handleToUpdateCreditNote = async () => {
    ...
    if (ReportListId !== null) {
      await updateappqc(Number(ReportListId));
      setAppqc("1");
      fetchData();
    }
    setData((prev: any) => ({
      ...prev,
      message: "บันทึกข้อมูลสำเร็จ-success",
    }));
    setTimeout(() => {
      setIsForEdit(false);
      setLoading(false);
