...
import { creditNoteToB1 } from "../services";
...
interface TableProps {
  ...
  loading: boolean;
  fetchData: () => void;
  setLoading: Dispatch<SetStateAction<boolean>>; // Add setLoading prop to control loading state
}
...
export default function CreditNoteTable({
  ...
  useEffect(() => {
    setSelectedCreditNotes([]); // Reset selection when creditNoteList changes
  }, [creditNoteList]);
  ...
  const handleToB1 = async () => {
    if (selectedCreditNotes.length != 0) {
      ...
      selectedCreditNotes.forEach(async (creditNote) => {
        try {
          await creditNoteToB1(creditNote.creditNoteId);
          // Refresh data after sending B1
        } catch (error) {
          console.error("Error sending B1:", error);
        }
      });
        setData((prev: any) => ({
      ...prev,
      ...
      <div className="overflow-hidden w-[100%] rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        >
          ...
          >
            <TableContainer
              component={Paper}
              sx={{ boxShadow: "none", background: "transparent",scrollbarWidth: 'thin',scrollbarColor: '#cbd5e1 #f3f4f6' }}
            >
              <Table>
                <TableHead>
                  <TableRow>
                ...
                <TableBody>
                  {paginatedData?.map((order: CreditNote) => {
                    const isSelected = selectedCreditNotes.some(
                      (item) => item.creditNoteId === order.creditNoteId
                    );
                    return (
                      <TableRow
                        ...
                        onClick={() => {
                          setSelectedCreditNotes((prev) => {
                            if (isSelected) {
                              ...
                              return prev.filter(
                                (item) =>
                                  item.creditNoteId !== order.creditNoteId
                              );
                            } else {
                              // Add to selection
                              return [...prev, order];
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 whitespace-nowrap">
                          ...
                          >
                            {
                              StatusFromPOS[
                                order.creditNoteStatus as keyof typeof StatusFromPOS
                              ]
                            }
                          </Badge>
                        </TableCell>
